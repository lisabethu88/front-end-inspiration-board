...
const CardList = ({ board }) => {
    const transformResponse = (card) => {
            const { card_id, message, board_id, likes_count } = card;
            return { card_id, message, board_id, likes_count };
        }
    /* ------------------- STATE ------------------- */
    ...
    const cards = cardsState.map((cardItem) => {
        return <Card card={cardItem} deleteCardCb={deleteCard} updateLikes={updateLikes}/>;
    });
